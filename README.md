...
| 飞书AI能力 | aily智能体(知识库+术语库) + 多维表格(8表54+条) + 妙搭低代码 |
...
| ✅ 多维表格 | 8表54+条记录(门店/巡检/客户/销售/产品库存/术语表/整改任务/区域月度统计) + 8个公式字段 | [打开](https://larkcommunity.feishu.cn/base/R7v9bKHNdaBsDFsDdvzcXICOnWh) |
...
├─────────────────────────────────────────────┤
│  数据层  飞书多维表格（轻量ERP）              │
│          8表54+条记录 / 门店/巡检/库存/客户/销售/术语表/整改任务/区域月度统计 │
├─────────────────────────────────────────────┤
│  接口层  飞书开放平台API                      │
...
| 数据存储 | 飞书多维表格 | 轻量ERP，云端8表54+条记录+AI字段 |
| 低代码 | 飞书妙搭 | AI生成巡检应用，1343行代码 |
| API | 飞书开放平台 | tenant_access_token + Bitable + Aily |
| 部署 | GitHub Pages + 飞书云 | 无需自建服务器 |

> **数据口径说明：** 飞书多维表格云端存储8表54+条真实记录（含8个公式字段）；本地 `data/` 目录仅存放mock演示数据（产品/门店/巡检记录），供无飞书凭证时Demo运行使用。
