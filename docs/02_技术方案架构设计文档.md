...
| 层级 | 定位 | 核心组件 | 职责说明 |
|------|------|---------|---------|
| 渠道层 | 渠道管理中枢 | 标准化看板、渠道洞察、合规排名与库存预警、经营洞察 | 站在总部/区域经理视角，汇总全渠道巡检数据、管理网点拓展、合规排名与库存预警、跨门店经营洞察 |
...
- **AI陈列分析**：巡检照片上传后，通过飞书集成平台AI节点自动分析陈列合规性，识别产品分区、货架利用率、价签覆盖率、卫生状况等维度；
...
- **数据底座**：飞书多维表格新增渠道管理表组（渠道网点表、订单分单表、库存预警表、渠道拓展档案表），与门店表、巡检记录表、销售数据表关联；
- **前端**：Flask新增 `/channel` 路由 + `channel.html` 页面，提供渠道标准化看板、渠道洞察、合规排名与库存预警、经营洞察四大视图。
...
| 能力 | 实现方式 | 参考数据 |
|------|---------|---------|
| 合规排名 | 多维表格自动化，根据网点位置/库存/配送能力自动分配订单 | 库存预警响应时间<1分钟，库存数据实时准确 |
| 履约进度实时监控 | 已下单→已发货→已到货→已施工全链路跟踪 | — |
| 超时预警 | 多维表格AI字段自动计算履约时长，超时自动推送预警 | — |
| 断货风险预警 | 关联库存表，库存低于安全线自动预警 | 库存数据实时准确，库存周转率3.8次/年 |
...
| 合规排名 | `POST /api/channel/auto_dispatch` | 根据网点位置/库存/配送能力自动分配订单 |
| 库存预警 | `GET /api/channel/fulfillment_monitor` | 监控订单履约进度，超时/断货预警 |
...
| 订单分单表（新增） | 订单编号、网点编号、下单时间、分单时间、订单状态、分单网点、履约状态、预计到货时间、超时标记 | 记录合规排名与库存预警数据 |
...
| 渠道洞察（新增） | 完全权限 | 本区域 | 无 | 仅自有档案 |
| 合规排名与库存预警（新增） | 全国监控 | 区域监控 | 查看本店 | 自有门店 |
...
| AI陈列分析 | 飞书集成平台AI节点 | 门店陈列照片合规性检测，识别产品分区、货架利用率、价签覆盖率 |
